...
function runStaticChecks({ pr, files }) {
  ...
  if (totalChanges > 500) {
    warnings.push(`⚠️ **Large PR detected**: ${totalChanges} lines changed across ${files.length} files. Consider splitting into smaller PRs.`);
  }

  // 2. Missing description
  if (!pr.body || pr.body.trim().length < 20) {
    warnings.push('⚠️ **Missing description**: Please add a description explaining what this PR does and why.');
  }

  // 3. Generic PR title
  const genericTitles = ['fix', 'update', 'changes', 'wip', 'test', 'misc'];
  if (genericTitles.some((t) => pr.title.toLowerCase().trim() === t)) {
    warnings.push(`⚠️ **Generic PR title**: "${pr.title}" is too vague. Use a descriptive title like "fix: resolve login crash on iOS 17".`);
  }

  // 4. No tests changed
  const hasTestFiles = files.some((f) => /test|spec|__tests__/i.test(f.filename));
  if (!hasTestFiles && files.length > 2) {
    warnings.push('⚠️ **No test files detected**: If your changes affect business logic, please add or update tests.');
  }
  ...
  if (bigFiles.length > 0) {
    ...
    warnings.push(`⚠️ **Large file changes**: Heavy modifications in: \`${names}\`. Ensure changes are well-documented.`);
  }
  ...
  if (lockChanged && !pkgChanged) {
    warnings.push('ℹ️ **Lockfile changed** without `package.json` modification. Verify this is intentional.');
  }
